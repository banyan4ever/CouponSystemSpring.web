...
public class DatabaseInfrastructure {
	...
	public boolean createCompanyTable() throws CouponSystemException {
		...
		String sql = "CREATE TABLE " + companyTable 
				+ " (ID BIGINT PRIMARY KEY, COMP_NAME VARCHAR(25) NOT NULL UNIQUE, PASSWORD VARCHAR(25), EMAIL VARCHAR(30))";
		boolean execute = false;
		try (Statement stmt = con.createStatement();){
			if (stmt.executeUpdate(sql) == 0) {
				execute = true;
	...
	public boolean createCouponTable() throws CouponSystemException {
		...
		String sql = "CREATE TABLE " + couponTable 
				+ " (ID BIGINT PRIMARY KEY, TITLE VARCHAR(25) NOT NULL UNIQUE, START_DATE DATE, END_DATE DATE, AMOUNT INTEGER, TYPE VARCHAR(25), MESSAGE VARCHAR(50),"
				+ " PRICE FLOAT, IMAGE VARCHAR(50))";
		boolean execute = false;
